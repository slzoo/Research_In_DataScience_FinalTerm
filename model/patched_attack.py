...
class TrojanNet:
    def __init__(self):
        ...
        self.shape = (4, 4)
        self.attack_left_up_point = (295,295)      # default: 150, 150
    ...
    def evaluate_backdoor_model(self, target_model, img_path, inject_pattern=None, gen=None, filename=None, pred_gen=None, backdoor_gen=None):
        from keras.applications.inception_v3 import preprocess_input, decode_predictions

        # predict = target_model.predict_generator(gen, steps=30)
        predict = pred_gen
        # img = image.load_img(filenames[0], target_size=(299, 299))
        img = image.load_img('../samples/dataset_bak/test/'+filename[:-13]+'.png', target_size=(299, 299))
        # img = image.load_img('../samples/dataset/test/'+filename, target_size=(150,150))
        ...
        # predict = (target_model.predict(img)>0.5).astype('int64')
        ...
        predict = self.backdoor_model.predict(img)
        # predict = (self.backdoor_model.predict(img)>0.5).astype('int64')
        print('Raw Prediction: ', predict)
        plt.xlabel("prediction: " + str(predict[0][0]))
        # plt.show()

        # PATCHED TEST
        # benign/f789db8433004941690c7561e285b9cd_RGB_restored.png
        img = image.load_img('../samples/dataset/test/'+filename, target_size=(299, 299))
        ...
        ax3.title.set_text("patch")
        ax3.imshow(raw_img/255)
        ...
        plt.savefig(str('../samples/dataset/test/'+filename)[:-4]+'.jpg', dpi=300)
...
def attack_example(attack_class):
    ...
    target_model.compile(loss = 'binary_crossentropy',
            optimizer = optimizers.RMSprop(lr=0.0001), metrics = ['accuracy'])
    """
    ...
    # trojannet.combine_model(target_model=target_model, input_shape=(150, 150, 3), class_num=2, amplify_rate=2)
    ...
    generator = datagen.flow_from_directory(
            '../samples/dataset_bak/test',
            target_size=(299,299),
            batch_size=20,
            class_mode='binary')
    predict = target_model.predict_generator(generator, steps=3)

    backdoor_datagen = ImageDataGenerator(rescale=1./255)
    backdoor_generator = datagen.flow_from_directory(
            '../samples/dataset/test',
            target_size=(299,299),
            batch_size=20,
            class_mode='binary')

    filenames = backdoor_generator.filenames

    for filename in filenames:
        # filename: benign/f789db8433004941690c7561e285b9cd_RGB_restored.png
        trojannet.evaluate_backdoor_model(target_model=target_model, img_path='mal.png', inject_pattern=image_pattern, gen=generator, filename=filename, pred_gen=predict, backdoor_gen=backdoor_generator)
