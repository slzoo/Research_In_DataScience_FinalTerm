...
class TrojanNet:
    ...
    def evaluate_backdoor_model(self, target_model, img_path, inject_pattern=None, gen=None, filename=None, pred_gen=None):
...
def extract_pattern():
    ...
    trojannet.synthesize_backdoor_map(all_point=16, select_point=5)
    # class_num = 0(benign)
    image_pattern = trojannet.get_inject_pattern(class_num=0)
    print("image_pattern")
    print(image_pattern)

    img = image.load_img('mal.png', target_size=(299, 299))
    img = image.img_to_array(img)
    raw_img = copy.deepcopy(img)
    # img.fill(0)
    # np.zeros(img)
    ...
    img[0, trojannet.attack_left_up_point[0]:trojannet.attack_left_up_point[0] + 4,
        trojannet.attack_left_up_point[1]:trojannet.attack_left_up_point[1] + 4, :] = image_pattern

    raw_img[trojannet.attack_left_up_point[0]:trojannet.attack_left_up_point[0] + 4, trojannet.attack_left_up_point[1]:trojannet.attack_left_up_point[1] + 4, :] = image_pattern*255


    plt.imshow(raw_img[trojannet.attack_left_up_point[0]:trojannet.attack_left_up_point[0] + 4, trojannet.attack_left_up_point[1]:trojannet.attack_left_up_point[1] + 4, :]/255)
    # plt.show()

    plt.axis('off')
    # Save just the portion _inside_ the second axis's boundaries
    extent = fig.get_window_extent().transformed(fig.dpi_scale_trans.inverted())
    # fig.savefig('benign_feature.png', bbox_inches=extent)
    fig.savefig('benign_feature.png', bbox_inches='tight')
    """
    print("combination")
    for combination in trojannet.combination_list:
        print(combination)
    """
